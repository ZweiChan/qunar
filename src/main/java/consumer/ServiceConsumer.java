...
public class ServiceConsumer {
    ...
    //zookeeper地址
    private String ZK_CONNECTION_STRING;
    //zookeeper连接最大等待时间
    ...
    // 构造器
    public ServiceConsumer(String zkConnection,int sessionTimeout,String path) {
        BasicConfigurator.configure();
        ZK_CONNECTION_STRING=zkConnection;
        ZK_SESSION_TIMEOUT=sessionTimeout;
        ZK_REGISTRY_PATH=path;
        ZooKeeper zk = connectServer(); // 连接 ZooKeeper 服务器并获取 ZooKeeper 对象
        if (zk != null) {
            watchNode(zk); // 观察服务节点的所有子节点并更新 urlList 成员变量
        }
    }
    ...
    private void watchNode(final ZooKeeper zk) {
        try {
            ...
            List<String> dataList = new ArrayList<>(); // 用于存放 某服务节点下 所有子节点中的数据
            for (String node : nodeList) {
                byte[] data = zk.getData(ZK_REGISTRY_PATH + "/" + node, false, null); // 获取 服务节点 的子节点中的数据
